...
import { ZXingScannerModule } from '@zxing/ngx-scanner'; // <--- BIBLIOTECA NOVA E MAIS ROBUSTA
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, LucideAngularModule, ButtonComponent, ZXingScannerModule], // <--- Importação Limpa
  template: `
    <div class="max-w-4xl mx-auto p-6 pb-20">
        ...
        <div class="grid gap-4">
          <div>
            ...
            <div class="flex gap-2">
                <input [(ngModel)]="currentProduct.gtin" placeholder="Ex: 789..." class="flex-1 p-2 rounded border border-gray-300 bg-gray-50">
          ...
          <div>
            ...
            <input [(ngModel)]="currentProduct.name" placeholder="Ex: Arroz Tio João" class="w-full p-2 rounded border border-gray-300">
          </div>
          
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-bold text-slate-500 mb-1">Preço (R$)</label>
              <input [(ngModel)]="currentProduct.price" placeholder="0,00" class="w-full p-2 rounded border border-gray-300">
            </div>
            <div>
              <label class="block text-xs font-bold text-slate-500 mb-1">Categoria</label>
              <select [(ngModel)]="currentProduct.category" class="w-full p-2 rounded border border-gray-300 bg-white">
      ...
      <div class="grid gap-4">
        <div *ngFor="let product of products" class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex justify-between items-center">
          <div>
          ...
          <div class="flex gap-2">
            <button (click)="editProduct(product)" class="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-full"><lucide-icon [img]="EditIcon" class="w-5 h-5"></lucide-icon></button>
            <button (click)="deleteProduct(product._id)" class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-full"><lucide-icon [img]="TrashIcon" class="w-5 h-5"></lucide-icon></button>
          </div>
        </div>
      </div>
    </div>
...
export class SellerProductsComponent implements OnInit {
  ...
  readonly PlusIcon = Plus; readonly TrashIcon = Trash2; readonly EditIcon = Edit; readonly SaveIcon = Save; readonly XIcon = X; readonly ScanIcon = Scan; readonly StopIcon = StopCircle;
  ...
  onCodeResult(resultString: string) {
    // Toca um bip de sucesso (opcional, via HTML5 Audio)
    // const audio = new Audio('assets/beep.mp3'); audio.play().catch(() => {});

    this.currentProduct.gtin = resultString;
    this.isScanning = false; // Fecha a câmera
    alert('Código lido: ' + resultString);
  ...
  resetForm() { this.currentProduct = { _id: null, name: '', price: '', category: 'Mercearia', gtin: '', image: '' }; this.isEditing = false; }
  ...
  saveProduct() {
    ...
    if (this.isEditing && this.currentProduct._id) {
      this.http.put(`https://mercadofacil-hrvh.onrender.com/api/products/${this.currentProduct._id}`, payload).subscribe(() => { this.loadProducts(); this.closeForm(); });
    } else {
      this.http.post('https://mercadofacil-hrvh.onrender.com/api/products', payload).subscribe(() => { this.loadProducts(); this.closeForm(); });
    }
  }

  editProduct(product: any) { this.currentProduct = { ...product }; this.isEditing = true; this.showForm = true; }
  deleteProduct(id: number) { if (confirm('Tem certeza?')) { this.http.delete(`https://mercadofacil-hrvh.onrender.com/api/products/${id}`).subscribe(() => this.loadProducts()); } }
}