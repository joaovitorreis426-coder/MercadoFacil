                ...
                <div class="grid gap-6 md:grid-cols-3">
                  <div>
                    ...
                    <input type="text" [(ngModel)]="currentProduct.name" 
                      class="w-full border-gray-300 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-indigo-500"
                      [class.border-red-500]="!currentProduct.name">
                  </div>
                  ...
                  <div>
                    ...
                    <input type="text" 
                      ...
                      (ngModelChange)="applyCurrencyMask($event)"
                      placeholder="0,00"
                      class="w-full border-gray-300 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-indigo-500">
                  </div>
            ...
            <div class="bg-white rounded-xl border shadow-sm overflow-hidden">
              @if (products.length > 0) {
                <table class="w-full text-sm text-left">
                  <thead class="bg-gray-50 text-gray-500"><tr><th class="p-4">Nome</th><th class="p-4">Preço</th><th class="p-4 text-right">Ações</th></tr></thead>
                  <tbody>
                    @for (product of products; track product._id) {
                      <tr class="border-b hover:bg-gray-50">
                        <td class="p-4 font-medium">{{ product.name }}</td>
                        <td class="p-4 font-bold">R$ {{ product.price }}</td>
            ...
            <div class="max-w-2xl bg-white rounded-xl shadow-sm border p-8">
              ...
              <div class="space-y-6">
                ...
                <div class="p-4 bg-indigo-50 rounded-lg border border-indigo-100">
                  ...
                  <p class="text-sm text-indigo-700 mb-3">Defina onde sua loja aparece no mapa.</p>
                  
                  <div class="flex items-center gap-3">
                    <app-button (click)="getLocation()" className="bg-indigo-600 text-white text-xs">
                      📍 Obter Localização Atual
                    </app-button>
                    @if (storeSettings.lat) {
                      <span class="text-xs text-green-600 font-bold">Localização definida! ({{storeSettings.lat}}, {{storeSettings.lng}})</span>
                    } @else {
                      <span class="text-xs text-red-500">Localização pendente</span>
                    }
                  </div>
                </div>
...
export class SellerDashboardComponent implements OnInit {
  ...
  currentProduct = { name: '', price: '', status: 'Ativo' };
  ...
  applyCurrencyMask(value: string) {
    ...
    // 2. Converte para numero e divide por 100 (para considerar centavos)
    let numberValue = (parseInt(onlyNumbers) / 100).toFixed(2);
    
    // 3. Se deu NaN (campo vazio), zera
    if (isNaN(parseFloat(numberValue))) numberValue = "0.00";

    // 4. Troca ponto por vírgula
  ...
  checkUserAndFetch() {
    ...
    this.storeSettings = {
      ...
      lat: user.lat, // Carrega lat do banco
      lng: user.lng  // Carrega lng do banco
    };
  ...
  updateStoreSettings() {
    ...
    this.http.put('http://localhost:3000/api/user/update-profile', {
      email: user.email,
      ...this.storeSettings // Envia nome, tipo, lat e lng
    }).subscribe({
      next: (res: any) => {
  ...
  saveProduct() {
    ...
    const priceNum = parseFloat(this.currentProduct.price.replace(',', '.'));
    if (priceNum < 0) {
  ...
  // ... (startEdit, deleteProduct, resetForm, logout igual ao anterior) ...
  ...
  resetForm() { this.showForm=false; this.currentProduct={name:'',price:'',status:'Ativo'}; this.checkUserAndFetch(); }
  openCreateForm() { this.showForm=!this.showForm; this.currentProduct={name:'',price:'',status:'Ativo'}; this.isEditing=false; }
