...
import { LucideAngularModule, MapPin, ShoppingCart, Search, Trash2, TrendingDown, Plus, Save, List, Pencil, XCircle, Trophy, Navigation } from 'lucide-angular';
      ...
      <header class="bg-white shadow-md sticky top-0 z-10 border-b border-blue-100">
        <div class="max-w-6xl mx-auto px-4 py-4 flex justify-between items-center">
          <h1 class="text-xl font-bold text-slate-800 flex items-center gap-2">
            <div class="bg-blue-100 p-2 rounded-lg text-blue-600"><lucide-icon [img]="CartIcon" class="w-6 h-6"></lucide-icon></div>
            Mercado Fácil
          </h1>
      ...
      <main class="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        @if (viewMode === 'saved') {
          <section class="animate-in fade-in">
             <h2 class="text-xl font-bold text-slate-700 mb-4 flex items-center gap-2"><lucide-icon [img]="ListIcon" class="text-blue-500"></lucide-icon> Minhas Listas</h2>
               ...
               @for (list of savedLists; track list._id) {
                 <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-200 hover:border-blue-300 transition-all group relative">
                    ...
                    <div class="flex items-center gap-2 mb-2">
                      <span class="text-[10px] font-bold uppercase px-2 py-1 rounded bg-blue-50 text-blue-600">{{ list.category }}</span>
                      <span class="text-[10px] font-bold uppercase px-2 py-1 rounded bg-gray-50 text-gray-600">{{ list.frequency }}</span>
                    </div>
                    <h3 class="font-bold text-lg text-slate-800">{{ list.name }}</h3>
                    <p class="text-sm text-slate-500 mb-4">{{ list.items.length }} itens</p>
                    <app-button (click)="loadListToCompare(list)" className="w-full justify-center bg-blue-600 text-white text-sm">Comparar Agora</app-button>
                 </div>
               }
             </div>
          </section>
        ...
        @if (viewMode === 'create') {
          <section class="bg-white p-6 rounded-xl shadow-sm border border-blue-100">
            <div class="flex justify-between items-center mb-6">
              ...
              <div class="flex gap-2">
                @if (editingId) { <button (click)="resetForm()" class="text-sm bg-gray-100 px-3 py-1.5 rounded-lg font-bold">Cancelar</button> }
                @if (shoppingList.length > 0) {
                  <button (click)="saveListToDb()" class="text-sm px-3 py-1.5 rounded-lg font-bold flex items-center gap-2 shadow-sm" [class.bg-green-100]="!editingId" [class.text-green-700]="!editingId" [class.bg-orange-100]="editingId" [class.text-orange-800]="editingId">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                @for (result of ranking; track $index) {
                  <div class="bg-white rounded-2xl shadow-md overflow-hidden border-t-4 flex flex-col transition-transform hover:-translate-y-1"
                       ...
                       [class.ring-2]="$index === 0" [class.ring-yellow-400]="$index === 0"> <div class="p-4 bg-gray-50 border-b border-gray-100 text-center">
                      @if ($index === 0) { 
                        <span class="inline-block bg-yellow-100 text-yellow-800 text-xs font-extrabold px-3 py-1 rounded-full mb-2">CAMPEÃO 🏆</span>
                      }
                    ...
                    <div class="p-3 bg-white border-t border-gray-100">
                      <button class="w-full py-2 rounded-lg text-sm font-bold text-white transition-colors"
                              [class.bg-green-600]="$index === 0" [class.hover:bg-green-700]="$index === 0"
                              [class.bg-slate-600]="$index > 0" [class.hover:bg-slate-700]="$index > 0">
                         {{ $index === 0 ? 'Escolher Esta Loja' : 'Ver Detalhes' }}
                      </button>
                    </div>
...
export class ConsumerListComponent implements OnInit {
  ...
  loadingLocation = false; // Estado de carregamento do GPS
  ...
  // ... (resetForm, startEditing, saveListToDb, deleteList, loadListToCompare IGUAIS ao anterior) ...
  // Copie essas funções do código anterior se precisar, elas não mudaram. 
  // Vou colocar resumido aqui para caber na resposta:
  ...
  addItem() { if(this.newItem) this.shoppingList.push(this.newItem); this.newItem = ''; this.suggestions = []; }
  removeItem(i: number) { this.shoppingList.splice(i, 1); }

  // --- NOVA FUNÇÃO DE COMPARAÇÃO COM GPS ---
  comparePrices() {
    if (this.shoppingList.length === 0) return;
    this.loadingLocation = true; // Mostra aviso carregando

    // Tenta pegar o GPS
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          this.sendComparisonRequest(position.coords.latitude, position.coords.longitude);
        },
        (error) => {
          console.warn('GPS negado ou erro:', error);
          this.sendComparisonRequest(null, null); // Manda sem GPS se der erro
        }
      );
    } else {
      this.sendComparisonRequest(null, null);
    }
  }
  ...
  sendComparisonRequest(lat: number | null, lng: number | null) {
    ...
    this.http.post<any[]>('https://mercadofacil-hrvh.onrender.com/api/compare', payload).subscribe({
      next: (data) => { 
        ...
        if (data.length === 0) alert('Nenhum produto encontrado nas lojas.'); 
      },
      error: () => { 
